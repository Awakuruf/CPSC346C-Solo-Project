...
| Cost envelope ≤ $1 / 10k predictions | Egress or model load underestimated | **Statement:** Cost per 10k must stay under \$1. **Risk:** If false, system unsustainable. **Evidence so far:** AWS calculator (API Gateway + Lambda + Dynamo). **Test method:** Back-of-envelope cloud cost estimate. **Acceptance criterion:** ≤ $1 per 10k. **Next action:** If fails, optimize cache + cheaper storage. | ~$0.40 / 10k | Holds for baseline; risk under spikes |
| Consent model for fine location | Users understand opt-in | **Statement:** Users must consent to fine-grained location. **Risk:** If false, non-compliance. **Evidence so far:** Copy review. **Test method:** Onboarding wording review. **Acceptance criterion:** ≥80% correct comprehension. **Next action:** Pilot test wording before rollout. | Pilot test pending | Must verify comprehension before enabling fine GPS |
